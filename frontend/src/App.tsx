  ...
  import { formatFreshness, priceDeviation, deviationLabel, sourceLabel, build2gisRouteUrl } from './utils';
  ...
  export default function App() {
    ...
    const debouncedClinicQuery = useDebounce(clinicQuery, 300);
    ...
    const [compareCitySearch, setCompareCitySearch] = useState("");
    ...
    useEffect(() => {
      if (!city) {
        ...
        setCompareItems(new Map<string, ServiceOffer>());
        setCompareCities([]);
        setSourceStats([]);
        return;
      }
    ...
    const baseFilterParams = useMemo(() => {
      ...
      if (debouncedClinicQuery.trim())
        p.set('clinic', debouncedClinicQuery.trim());
      return p;
    }, [debouncedQuery, debouncedClinicQuery, selectedServiceId, category, selectedSources, priceMin, priceMax, ratingMin, onlineOnly]);
...
const toggleCompare = useCallback((offer: ServiceOffer) => {
    ...
    const runCompare = useCallback(() => {
        const selected = Array.from(compareItems.values());

        if (selected.length < 2) {
            alert("Выберите минимум две услуги.");
            return;
        }

        const serviceIds = [...new Set(selected.map(o => o.service_id))];

        if (serviceIds.length !== 1) {
            alert("Можно сравнивать только одинаковые услуги.");
            return;
        }
  ...
  const q = compareCitySearch.trim().toLowerCase();

  return compareCityOptions
    .filter(c => !compareCities.includes(c))
  ...
  <section className="city-gate">
    <div className="control-group city-gate-select">
      <label>
        1. Выберите город ({cities.length || "…"} с данными)
      </label>
          ...
          {!filteredCities.length ? (
            ...
            filteredCities.map(c => (
              <button
                key={c}
                type="button"
                className={`city-search-item ${city === c ? "active" : ""}`}
                onClick={() => {
                  setCity(c);
                  setCitySearch(c);
                  setCompareCities([]);
    ...
    {!city && (
      <p className="city-gate-hint">
        Показаны только города, где есть актуальные предложения.
        Начните вводить название города или выберите его из списка.
      </p>
    )}
  </section>
    ...
    <div className="control-group city-gate-select">
      ...
      <div className="city-search-select">
        ...
        <input
          className="city-search-input"
          type="text"
          value={compareCitySearch}
          placeholder="Начните вводить название города..."
          onChange={(e) => setCompareCitySearch(e.target.value)}
        />

        {compareCitySearch && (
          <div className="city-search-list">

            {!filteredCompareCities.length ? (
              <div className="city-search-empty">
                Город не найден
              </div>
            ) : (
              filteredCompareCities.map(c => (
                <button
                  key={c}
                  type="button"
        ...
        {compareCities.map(c => (
          ...
          <div className="control-group">
            ...
            <input
              ...
              placeholder="Например, Инвитро, KDL, Олимп..."
            />
          </div>  
              ...
              <section className="toolbar">
                ...
                {compareItems.size >= 2 && (
                  <button type="button" className="primary-button" onClick={runCompare}>Сравнить ({compareItems.size})</button>
                )}
                {compareItems.size > 0 && (
                    <button
                        className="secondary-button"
                        onClick={() => {
                            setCompareItems(new Map());
              ...
              <section className="results" ref={resultsRef}>
                ...
                <div className="grid" key={`page-${page}`}>
                  {offers.map(offer => {
                    const key = offerKey(offer);
                    const isCompared = compareItems.has(key);
                    ...
                    return (
                      <article key={`${key}-${offer.city}`} className={`card ${isCompared ? 'card-selected' : ''}`}>
                        ...
                        <div className="clinic-actions">
                          ...
                          {offer.address && (
                            <a
                              href={build2gisRouteUrl(offer.address, offer.city, offer.location, userLocation ?? undefined)}
                              target="_blank"
                              rel="noreferrer"
                            >
                              Маршрут
